...
module.exports = {
  ...
  Query: {
     ...
     *
     */
    async workspaces (_, args, { user }) {
      if (user?.admin === true) {
        return Workspace.find()
      }
      return Workspace.find({ members: user?._id }).lean()
    },
  },
  ...
  Workspace: {
    ...
    async member (workspace, { userId }) {
      const member = workspace.members.find((id) => String(id) === userId)
      return new WorkspaceArticle(workspace, member)
    },

    async members (workspace, { limit }) {
      await workspace.populate({ path: 'members', limit }).execPopulate()
    ...
    async leave (workspace, args, { user }) {
      ...
      workspace.members.pull({ _id: user.id })
    ...
    async inviteMember (workspace, { userId }) {
      ...
      workspace.members.push({ _id: userId })
      return workspace.save()
    },
  }
}
