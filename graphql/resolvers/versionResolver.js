...
module.exports = {
  // FIXME:
  // Article en cours a des données : texte, métadonnées
  // Les versions sont des snapshots à un instant T d'un article (clone)
  // La liste des versions ne devrait pas contenir une Version "en cours", c'est l'article qui devrait avoir une état courant.

  // Dans la logique, lorsqu'on édite l'article (en cours) il faut enregistrer les changements saveArticle
  // Quand on enregistre une nouvelle version, on fait un appel à saveVersion
  // Il n'est pas possible de mettre à jour une version (historique).

  // Dans la liste des versions, on a donc tout le temps :
